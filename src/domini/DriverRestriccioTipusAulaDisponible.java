...
class DriverRestriccioTipusAulaDisponible {
	private static inout io;
	private static RestriccioTipusAulaDisponible r;
	...
	public static void testConstructor() throws Exception {
		...
		r = new RestriccioTipusAulaDisponible(id,tipus,activat);
	...
	public static void testConstructor2() throws Exception {
		...
		TipusRes tipus = io.readTipusRes();
		io.write("Introdueix Activat (Boolean): \n");
		r = new RestriccioTipusAulaDisponible(id,tipus);
		io.write("La Restriccio creada te: \n Id = " + r.getId() + "\n TipusRestriccio = " + (r.getTipus().name()) + "\n");
	}
	...
	public static void testComprovar1(Sessio s) throws Exception {
		if (r.getTipus() == TipusRes.unaria) {
			...
			if (correcte) io.write("Hi ha algun tipus d'aula que concorda amb el tipus de sessio necessari \n");
			else io.write("NO hi ha cap tipus d'aula que concordi amb el tipus de sessio necessari \n");
		}
		else io.write("La restriccio ha de ser unaria. Per provar-la canvia el tipus a traves d'un set\n ");
	}
	
	public static void testComprovar2(Sessio s) throws Exception {
		if (r.getTipus() == TipusRes.unaria) {
			io.write("Creem una assignatura FM...\n");
			io.write("Creem un grup 10, de capacitat 30 persones, i l'assignem a FM...\n" );
			io.write("Creem una aula A01, de capacitat 30 persones, amb tipus aula LAB...\n");
			io.write("Creem una sessio de LAB per dilluns de 8h a 9h, pel grup 10 de FM a l'aula A01...\n");			
	...
	public static void main(String[] args) throws Exception {
		...
		au.setTipus(idcjTA2); //assigno tipus idcjTA a tipus de l'aula		
		...
		TipusSessio ts2 = new TipusSessio("LAB", idcjTA2, 2); //creo tipus sessio de TEO amb tipus Aula idcjTA
		Integer idcjTS2 = cjTS.NewId(ts2); //obtinc id del tipus sessio TEO dins del cjTS
		Vector<Integer> tsessions2 = new Vector<Integer>();  //creo tsessions
		tsessions2.add(idcjTS2); //afegeixo id del tipus sessio Teoria del cjTS a tsessions
		...
		Assignatura a2 = new Assignatura ("FM", "Fonaments Matematics", 1, tsessions2); //creo assignatura amb sessions tsessions
		Integer idcjA2 = cjA.NewId(a2); //obtinc id assig dins del cjA
		...
		GrupAssig ga2 = new GrupAssig (idcjG, idcjA2, true); //creo Grup assig per assig idA, grup idcjg
		Integer idcjGA2 = cjGA.NewId(ga2); //obtinc id del grupassig dins del conjunt cjGA
		Sessio ss = new Sessio("sessio1", idcjAU, idcjGA, idcjTS); //creo sessio per l'aula idcjAU, pel grupassig idcjGA, pel tipus de sessio idcjTS
		Sessio ss2 = new Sessio("sessio2", idcjAU, idcjGA2, idcjTS2); //creo sessio per l'aula idcjAU, pel grupassig idcjGA, pel tipus de sessio idcjTS
		io.write("Opcions: \n 1.Constructor \n 2.Getters \n 3.Setters \n 4.Metode \n 5.Exit \n");
		while (s != 6) {
			if (s == 1) {
				io.write("Opcions: \n 1.Constructora 1 \n 2.Constructora 2 \n 3.Endarrera \n");
				...
				if (r == null) io.write("S'ha de crear una restriccio abans de comprovar"); 
					...
					s = io.readint();
					if (s == 1) testComprovar1(ss);
					if (s == 2) testComprovar2(ss2);
				}
				io.write("\n");
				io.write("Opcions: \n 1.Constructor \n 2.Getters \n 3.Setters \n 4.Metode \n 5.Exit \n");
			}
